...
Error_Table_Df.to_excel('Error_Table_Df_Parth_ID2693653.xlsx', index=True)
...
# commented for the folding plot generation
"""
...
knn_error_rate_list = np.zeros(len(k_range))
...
for index, k in enumerate(k_range):
    ...
    knn_error_rate_list[index] = test_error

mknn_error_rate_list = np.zeros(len(k_range))

for index, k in enumerate(k_range):
    predicted_labels = mknn_classifier_ID2693653(train_split_data, train_split_true_labels, test_split_data, k)
    test_error = np.mean(predicted_labels != test_split_true_labels)
    mknn_error_rate_list[index] = test_error

fuzzy_knn_error_rate_list = np.zeros(len(k_range))

m=2
for index, k in enumerate(k_range):
    predicted_labels = fuzzy_knn_classifier_ID2693653(train_split_data, train_split_true_labels, test_split_data, k, m)
    test_error = np.mean(predicted_labels != test_split_true_labels)
    fuzzy_knn_error_rate_list[index] = test_error

plt.plot(list(k_range), knn_error_rate_list, label="KNN Error Rate", marker='o', linestyle='-', color='blue')
plt.plot(list(k_range), mknn_error_rate_list, label= "Modified KNN Error Rate", marker='o', linestyle='-', color='green')
plt.plot(list(k_range), fuzzy_knn_error_rate_list, label="Fuzzy KNN Error Rate", marker='o', linestyle='-', color='red')
