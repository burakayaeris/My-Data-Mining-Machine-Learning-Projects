...
def rnn_classifier_ID2693653(train_split_data, train_split_true_labels, test_split_data, r):
    ...
    for i in range(len(test_split_data)):
        ...
        if len(neighborhood_indices) == 0:
            ...
            neighborhood_labels = train_split_true_labels.iloc[neighborhood_indices].to_numpy()
            unique_labels, counts = np.unique(neighborhood_labels, return_counts=True)
            test_predicted_labels[i] = unique_labels[np.argmax(counts)]
